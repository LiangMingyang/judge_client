...
(function() {
  var FILE_PAGE, MAX_DETAIL_LENGTH, NoTask, PipeError, Promise, REPORT_PAGE, TASK_PAGE, URL, child_process, child_process_promised, crypto, data_dirname, fs, judge_client, log, os, path, promisePipe, resource_dirname, rp, submission_dirname, utils_dirname, work_dirname,
  ...
  PipeError = (function(superClass) {
    extend(PipeError, superClass);

    function PipeError(message) {
      this.message = message != null ? message : "PipeError.";
      this.name = 'PipeError';
  ...
  judge_client = (function() {
    ...
    judge_client.prototype.pre_submission = function() {
      var data, i, inputFiles, j, outputFiles, ref, test_setting, time_limit_cases, weights, work_path;
      test_setting = "";
      ...
      time_limit_cases = [];
      for (i = j = 1, ref = Math.max(1, self.task.test_setting.data.length); 1 <= ref ? j <= ref : j >= ref; i = 1 <= ref ? ++j : --j) {
